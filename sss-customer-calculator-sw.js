...
function computeDIYComparison(proTotal) {
  ...
  // Exotic Timber Oil (oil-essential tier) is sold by the GALLON at the
  // Sherwin-Williams list price, not in 5-gal pails. When that's the
  // selected product the DIY stain line is priced per gallon at this
  // rate and flagged with an "SW List Price" tag.
  const SW_TIMBER_OIL_PER_GAL = (typeof D.timberOilPerGal === 'number') ? D.timberOilPerGal : 96.99;
  let timberOilGallons = 0;   // gallons priced at SW list (Timber Oil)
  let nonTimberPails   = 0;   // 5-gal pails of everything else
  let usedTimberOil    = false;
  ...
  projects.forEach(p => {
    ...
    // Timber Oil = oil-essential tier. Sold per gallon at SW list price.
    const isTimberOil = (p.productType === 'oil' && p.tier === 'essential');
    const gallonsForProj = pails * 5;
    const stainCostForProj = isTimberOil
      ? gallonsForProj * SW_TIMBER_OIL_PER_GAL
      : pails * pailCost;
    if (isTimberOil) { timberOilGallons += gallonsForProj; usedTimberOil = true; }
    else { nonTimberPails += pails; }
    ...
    const stainDesc = isTimberOil
      ? `${gallonsForProj} gal × $${SW_TIMBER_OIL_PER_GAL} stain (SW List Price)`
      : `${pailsLabel} × $${pailCost} stain`;
  ...
  // Stain row(s). Timber Oil is shown per-gallon at the SW list price
  // with a tag; everything else stays on the 5-gal-pail model. A mixed
  // quote (Timber Oil + another product) splits into two rows.
  let stainRowsHtml;
  if (usedTimberOil && nonTimberPails === 0) {
    stainRowsHtml = `<div class="diy-row"><span>Stain (${timberOilGallons} gal × $${SW_TIMBER_OIL_PER_GAL}/gal) <span class="diy-tag">SW List Price</span></span><span>$${Math.round(totalStainCost).toLocaleString()}</span></div>`;
  } else if (usedTimberOil && nonTimberPails > 0) {
    const timberCost = Math.round(timberOilGallons * SW_TIMBER_OIL_PER_GAL);
    const otherCost  = Math.max(0, Math.round(totalStainCost - timberCost));
    stainRowsHtml = `<div class="diy-row"><span>Exotic Timber Oil (${timberOilGallons} gal × $${SW_TIMBER_OIL_PER_GAL}/gal) <span class="diy-tag">SW List Price</span></span><span>$${timberCost.toLocaleString()}</span></div>`
      + `<div class="diy-row"><span>Other stain (${nonTimberPails} × 5 gal pails @ non-contractor pricing)</span><span>$${otherCost.toLocaleString()}</span></div>`;
  } else {
    stainRowsHtml = `<div class="diy-row"><span>Stain (${totalPails} × 5 gal pails @ non-contractor pricing)</span><span>$${totalStainCost.toLocaleString()}</span></div>`;
  }
