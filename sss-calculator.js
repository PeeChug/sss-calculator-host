...
async function loadCustomerSubmissions() {
  const panel  = __doc.getElementById('custSubsPanel');
  ...
  try {
    ...
    __custSubsCache = Array.isArray(res.items) ? res.items : [];
    if (countEl) countEl.textContent = String(__custSubsCache.length);
    if (__custSubsCache.length === 0) {
      // Hide entirely when there are no customer-side submissions yet.
      panel.style.display = 'none';
      return;
    }
    panel.style.display = 'block';
    ...
    // Hook up the local search input once.
    const searchEl = __doc.getElementById('custSubsSearch');
    if (searchEl && !searchEl._wired) {
      searchEl._wired = true;
      searchEl.addEventListener('input', (e) => {
...
function renderCustomerSubmissions() {
  ...
  if (countEl) countEl.textContent = q ? `${filtered.length}/${all.length}` : String(all.length);
  if (filtered.length === 0) {
    body.innerHTML = `<div class="folder-empty">No customer submissions match "${escapeHtml(q)}".</div>`;
    return;
  }
...
// Toggle handler so the open/closed state of the customer-submissions
// folder is reflected in CSS the same way the regular folders work.
function onCustSubsToggle(e) {
  // <details> handles its own toggle — this hook is just here in case
  // we ever want to track open state in dashState.
}
