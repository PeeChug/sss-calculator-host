  ...
  try {
    ...
    if (data && data.ok) {
      ...
      const openLink = data.jobberWebUri
        ? `<a href="${escapeHtml(data.jobberWebUri)}" target="_blank" rel="noopener" class="btn btn-primary" style="margin-top:14px;font-size:15px;padding:12px 22px;text-decoration:none;display:inline-flex;align-items:center;gap:6px;margin-right:8px;box-shadow:var(--shadow-md);">↗ Open in Jobber to Send</a>`
        : '';
