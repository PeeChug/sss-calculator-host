...
function startHeroMarquees() {
  ...
  // Match the CSS breakpoint (was 641, now 900 — phones in landscape
  // and Wix preview 'mobile' often report > 640px so the marquee was
  // never activating).
  const isMobile = window.innerWidth < 900;
  console.log('[Customer] hero marquee: viewport ' + window.innerWidth + 'px → ' + (isMobile ? 'mobile (marquee on)' : 'desktop (static grid)'));
  if (!isMobile) return;

  // Reduced motion — let CSS skip animation; nothing to wire on touch.
  try {
    if (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
      console.log('[Customer] reduced-motion — skipping gallery marquee');
      return;
    }
  } catch (e) {}

  const rows = __doc.querySelectorAll('.cust-hero-gallery-row[data-marquee]');
  console.log('[Customer] hero marquee: found ' + rows.length + ' rows to animate');
  rows.forEach(function (row) {
    ...
    row.addEventListener('touchcancel', resumeSoon, { passive: true });
  });
}
