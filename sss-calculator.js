...
function renderPricingAdminTab(tab) {
  ...
  if (tab === 'reps') { setTimeout(loadAndRenderReps, 0); return '<div id="repsTabBody" style="padding:8px 0;color:var(--slate);font-size:13px;">Loading reps…</div>'; }
  if (tab === 'devices') { setTimeout(loadAndRenderDevices, 0); return '<div id="devicesTabBody" style="padding:8px 0;color:var(--slate);font-size:13px;">Loading devices…</div>'; }
  return '';
}
...
async function loadAndRenderReps() {
  ...
  try {
    ...
    body.innerHTML = `<div class="folder-empty" style="color:var(--coral);">Couldn't load reps: ${escapeHtml(e.message || e)}</div>`;
  }
}
...
async function loadAndRenderDevices() {
  ...
  try {
    ...
    body.innerHTML = `<div class="folder-empty" style="color:var(--coral);">Couldn't load devices: ${escapeHtml(e.message || e)}</div>`;
  }
}
