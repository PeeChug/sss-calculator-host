...
function deepMerge(target, source) {
  // Plain-object recursive merge — arrays from source replace target.
  // Used for the rules patch (numbers + nested objects only).
...
function compactRulesOverride(node, defaultsRoot, path) {
  ...
  Object.keys(node).forEach(k => {
    const v = node[k];
    // Build the corresponding default-tree path.
