...
function renderDashAnalytics(stats) {
  ...
  const emptyState = (t.allTime || 0) === 0 && !(stats.funnel && stats.funnel.drafts > 0)
    ? '<div class="cust-analytics-empty">No customer submissions in the last 90 days yet. This fills in as the public calculators get used.</div>'
    : '';
  ...
  body.innerHTML = `
    ...
    ${emptyState || (kpiHtml + splitHtml + weeklyHtml + mixHtml + funnelHtml + v2Missing)}`;
}
...
const PA_GROUPS = {
  pricing: ['tiers', 'prep', 'extras', 'addons', 'discounts', 'diy', 'quote'],
  team:    ['reps', 'devices']
};
