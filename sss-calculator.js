...
function computeDIYComparison(proTotal) {
  ...
  const PROJECT_TIME_DIVISOR = { fence: 16, deck: 24, pergola: 12, barn: 18, ceiling: 15 };

  // Retail pail price (5 gal) by product family + tier. Prices are
  // approximate non-contractor retail — the homeowner-walks-into-the-store
  // price — based on each product's actual retail SKU pricing.
  function pailCostFor(p) {
    if (p.productType === 'water') {
  ...
  const SODIUM_METASILICATE_PER_PAIL = 90;   // 5 gal cleaner powder
  const OXALIC_ACID_PER_PAIL         = 90;   // 5 gal brightener powder
  ...
  const PRESSURE_WASHER_COST = 199;
...
function finalizeQuote(sendMethod) {
  ...
  if (sendMethod === 'open') {
    try {
      ...
      // Paint a friendly "Creating quote…" page into the placeholder
      // so the rep doesn't stare at a blank tab while the create
      // mutation + attachment shotgun run (typically 3–8 seconds).
      // When the push completes, response.handler swaps the URL.
      if (state.__jobberTabRef) {
        try {
          // Build the loader page from string parts. We deliberately
          // assemble the closing tags via concatenation — the build
          // script's body-extraction regex is non-greedy on its body
          // close-tag boundary and would otherwise stop dead at a
          // literal close-tag inside this template, chopping off the
          // rest of the calc's JS. (Yes, this bit us once.)
          const _close = '<' + '/body><' + '/html>';
          state.__jobberTabRef.document.write(
            '<!DOCTYPE html><html><head>' +
            '<meta charset="utf-8">' +
            '<title>Creating quote in Jobber…</title>' +
            '<style>' +
              "body{margin:0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:#f8f9fb;color:#1a2540;height:100vh;display:flex;align-items:center;justify-content:center;}" +
              '.box{text-align:center;max-width:380px;padding:32px;}' +
              '.spinner{width:48px;height:48px;border:4px solid #e6e8ec;border-top-color:#4a8a64;border-radius:50%;margin:0 auto 24px;animation:spin 1s linear infinite;}' +
              '@keyframes spin{to{transform:rotate(360deg);}}' +
              'h1{font-size:18px;margin:0 0 8px;}' +
              'p{font-size:14px;color:#5a6378;margin:0;line-height:1.5;}' +
              'small{display:block;margin-top:18px;font-size:11px;color:#7a8398;}' +
            '</style>' +
            '</head><body>' +
              '<div class="box">' +
                '<div class="spinner"></div>' +
                '<h1>Creating your quote in Jobber…</h1>' +
                "<p>This usually takes a few seconds. We'll redirect you automatically once it's ready.</p>" +
                '<small>Tab opened by Superior Stain Solutions calculator</small>' +
              '</div>' +
            _close
          );
          state.__jobberTabRef.document.close();
        } catch (e) { /* cross-origin or write failed — placeholder stays blank, no big deal */ }
      }
...
async function pushFinishedQuoteToJobber(rowId, force, sendMethod) {
  ...
  if (box) {
    const verb = sendMethod === 'open' ? 'Saving draft to Jobber, then opening it for you…'
               : sendMethod === 'none' ? 'Saving draft to Jobber…'
               : force ? 'Re-sending draft to Jobber…' : 'Saving draft to Jobber…';
  ...
  try {
    ...
    if (data && data.ok) {
      ...
      if (Array.isArray(data.sentLineItems) && data.sentLineItems.length) {
        ...
        let sendMethodPill = '';
        if (data.sendMethod === 'open') {
          sendMethodPill = pill('#e8f5ff', '#0b69b8', '✓ Saved as Draft & opened in Jobber — hit Send there');
        } else if (data.sendMethod === 'none') {
...
function buildJobberLineItem(p, idx, total) {
  ...
  const lines = [];
  // Simple section helper: blank line + heading (with emoji acting as
  // the visual cue). The earlier ASCII divider lines were redundant
  // visual noise — the emoji heading is enough to break up sections,
  // and the description reads tighter without "───────────" twice
  // around every block.
  ...
  if (p.woodAge || p.condition) {
    ...
    lines.push(`🌧 Condition & prep: ${condBits.join(' · ')}`);
  }
  ...
  if (p.selectedDiscounts && p.selectedDiscounts.length) {
    ...
    if (discountList.length) {
      pushSection('🏷️ Discounts applied');
  ...
  if (p.previousStain && p.previousStain.wasStained) {
    ...
    pushSection('ℹ️ Previously stained');
  ...
  // Reference photos — append clickable URLs at the bottom of the
  // description. We also try to attach as proper Jobber line item
  // images in the backend (best-effort), but Jobber's GraphQL has
  // been removing attachment APIs over time so the URLs in-line are
  // the guaranteed-visible fallback. Jobber renders bare URLs as
  // clickable links in both the customer-facing quote and the PDF.
  const photos = (p.referencePhotos || []).filter(ph => ph && ph.url);
  if (photos.length) {
    pushSection(`📷 Reference photos (${photos.length})`);
    photos.forEach((ph, i) => {
  ...
  return {
    ...
    // Surface raw photo URLs alongside the description so the backend
    // can also try a native Jobber line-item-image attachment after
    // the quote create lands.
...
function renderQRow(item, folder) {
  ...
  const by    = item.lastEditedBy || item.employeeName || '';
...
function paField(path, step) {
  ...
  const changed  = (typeof override === 'number') && override !== def;
  return `<input type="number" step="${step || 'any'}" data-pa-path="${path}" value="${value != null ? value : ''}" class="${changed ? 'pa-changed' : ''}" placeholder="${def != null ? def : ''}">`;
}
