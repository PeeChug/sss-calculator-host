...
function toggleCustHelpMenu() {
  ...
  if (menu.classList.contains('open')) {
    ...
    // breakpoint). We explicitly set its `top` to sit just below the
    // BOTTOM of the entire top chrome — floating bar + progress strip
    // stack — so the menu never gets covered by the progress steps
    // or pushed off-screen by overflow clipping.
    try {
      const isMobile = (typeof window !== 'undefined' && window.innerWidth < 641);
      if (isMobile) {
        const floatingBar = __doc.getElementById('custFloatingBar');
        const progressEl  = __doc.getElementById('progress');
        // Use whichever piece of top chrome ends LOWEST in the viewport
        // (progress strip when visible, otherwise the floating bar).
        let bottomY = 0;
        if (progressEl && progressEl.offsetParent !== null) {
          bottomY = Math.max(bottomY, progressEl.getBoundingClientRect().bottom);
        }
        if (floatingBar && floatingBar.offsetParent !== null) {
          bottomY = Math.max(bottomY, floatingBar.getBoundingClientRect().bottom);
        }
        // Fall back to the help-button bottom if both bars are mis-
        // measured (e.g. shadow DOM quirks).
        if (!bottomY && btn) bottomY = btn.getBoundingClientRect().bottom;
        menu.style.top = (bottomY + 8) + 'px';
