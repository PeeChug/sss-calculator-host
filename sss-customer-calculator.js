...
function startHeroMarquees() {
  ...
  // Animation now runs on every viewport (mobile, tablet, desktop) —
  // CSS handles the smooth drift. JS just wires touch pause/resume
  // so the user can hold a row to inspect it.
  const rows = __doc.querySelectorAll('.cust-hero-gallery-row[data-marquee]');
  console.log('[Customer] hero marquee: ' + rows.length + ' rows wired (viewport ' + window.innerWidth + 'px)');
  rows.forEach(function (row) {
    ...
    // Desktop hover too — pause on mouse-over so users can look closer.
