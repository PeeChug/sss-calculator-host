...
async function loadJobberRequests() {
  ...
  try {
    const r = await fetch('/_functions/jobberRequests?limit=10', { credentials: 'include' });
    ...
    body.innerHTML = nodes.map((req, i) => {
      ...
      return `
        <div class="req-card">
          ...
          <div class="req-card-actions">
            <button class="btn btn-primary" onclick="convertJobberRequestToQuote(${i})">＋ Convert to Quote</button>
          </div>
        </div>`;
    }).join('');
    panel.style.display = 'block';
