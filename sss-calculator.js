...
function computeDIYComparison(proTotal) {
  ...
  const diyTotal = totalStainCost + totalCitronellaCost + sprayerCost + totalToolsCost + timeCost + totalAddonMaterial;
...
function finalizeQuote() {
  ...
  (async () => {
    try {
      ...
      pushFinishedQuoteToJobber(state.cloudRowId);
...
async function pushFinishedQuoteToJobber(rowId, force) {
  ...
  if (box) {
    box.innerHTML = '<div class="jobber-push-row pending"><span class="ico">⏳</span><span>' +
      (force ? 'Re-sending to Jobber…' : 'Pushing to Jobber…') + '</span></div>';
  }
  try {
    const r = await fetch('/_functions/pushToJobber', {
      ...
      credentials: 'include',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ quoteRowId: rowId, force: !!force })
    });
    const data = await r.json();
