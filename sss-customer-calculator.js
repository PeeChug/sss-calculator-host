...
// =============================================================
// On phones each .cust-hero-gallery-track is animated by CSS
// @keyframes (translateX 0 → -50% for row 1, -50% → 0 for row 2).
// This JS just does the two things CSS can't:
//   1. Duplicate the tiles inside each track once at startup so the
//      -50% endpoint visually matches the 0% startpoint (seamless loop).
//   2. Toggle a .paused class on the row when the user touches it, so
//      the CSS animation-play-state pauses for that row only. Resumes
//      ~1.5s after touch ends so the customer can take a closer look.
function startHeroMarquees() {
  // Desktop doesn't run the marquee — gallery is a static 2-row grid.
  ...
  rows.forEach(function (row) {
    ...
    // Duplicate the existing tiles once. After this, the track has
    // 2x the original content; CSS translates -50% which == one full
    // copy of the originals, so the visual at -50% is identical to
    // the visual at 0% (seamless wrap).
    ...
    // Touch handlers: pause this row while the user is interacting,
    // resume after a short delay so they can read details if needed.
    ...
    row.addEventListener('touchend',    resumeRow, { passive: true });
    row.addEventListener('touchcancel', resumeRow, { passive: true });
  });
}
