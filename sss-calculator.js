...
function finalizeQuote() {
  ...
  // Pull the parent page back up so the user sees the "Quote sent" screen
  // instead of being stranded mid-scroll on the review step.
  scrollAppToTop();
  // Hide the side panel + pill since the quote is locked in now.
  closeSideTracker();
  if (typeof setSavePill === 'function') setSavePill('hidden');
}
...
function renderFolder(folder) {
  ...
  return `
    <details class="folder" ${open ? 'open' : ''} data-folder="${folder}">
      ...
      <div class="folder-body">${bodyHtml}</div>
    </details>`;
}
...
function resumeCloudQuote(rowId) {
  ...
  __sssBridge.call('getQuote', { quoteRowId: rowId }).then(res => {
    ...
    showStage(state.maxStageReached || 1);
    updateRunningTotal();
    // After hydrate the side tracker tab visibility logic in renderSidePanel
    // can lag behind the new state by one render — force a second pass on
    // the next tick so the tab appears immediately on resumed quotes.
    setTimeout(() => {
      try {
        if (typeof renderSidePanel === 'function') renderSidePanel();
        if (typeof renderFinalBreakdown === 'function' && state.maxStageReached >= 10) {
          renderFinalBreakdown();
        }
        const tab = __doc.getElementById('sideTrackerTab');
...
function hydrateStateFromCloud(q) {
  ...
  state.maxStageReached = projs.length > 0 ? 10 : 1;
...
(function setupTapGuard() {
  ...
  Object.assign(window, { nextStage, prevStage, showStage, addAnotherProject, cancelAddProject, cancelEditBundled, collapseActiveProject, editBundledProject, removeBundledProject, resetQuote, startNewQuote, finalizeQuote, generatePDF, returnToDashboard, cancelNewQuote, clearAllDrafts, resumeDraft, deleteDraft, saveAndReturnToDashboard, onFolderToggle, onDashSearchInput, openRowMenu, closeRowMenu, resumeCloudQuote, resumeLocalDraft, deleteLocalDraft, moveCloudQuote, duplicateCloudQuote, permanentlyDeleteCloud, openSideTracker, closeSideTracker, clearTrackerRow, openInfoModal, closeInfoModal, openMeasureTutorial, closeMeasureTutorial, setProduct, setTier, toggleAddonInline, setAddonInlineQty, toggleEditPanel, applyCustomColor, removeCustomAddon, state });
