...
// =============================================================
// On phones each .cust-hero-gallery-row becomes a continuous horizontal
// scroller that drifts on its own (row 1 right-to-left, row 2 left-to-
// right). Touch a row to pause its scroll while you swipe through; it
// resumes 2 seconds after touchend. The loop is seamless because we
// duplicate the tiles once at startup so the row's content is 2× wide,
// then reset scrollLeft from `half` back to `0` (or vice versa for the
// reverse direction) — the duplicated tiles make the jump invisible.
function startHeroMarquees() {
  // Desktop / tablet doesn't need the marquee — the gallery already
  // shows all 6 photos in a tidy 2-row grid.
  ...
  rows.forEach(function (row) {
    const direction = row.getAttribute('data-marquee') === 'reverse' ? -1 : 1;
    // Different speeds per row so the page doesn't feel mechanically
    // synchronized. Pixels per second.
    const speed = (direction === 1 ? 22 : 17) * direction;
    startRowMarquee(row, speed);
  });
}

function startRowMarquee(row, pxPerSec) {
  if (!row) return;
  // Duplicate the children once so the row has 2× content. The loop
  // reset (scrollLeft -= half) lands on the IDENTICAL tile at the same
  // visual position thanks to the duplicates — no visible jump.
  ...
  // For the reverse-scrolling row, START in the middle so we have room
  // to scroll left before hitting 0 (browser can't go below 0).
  function getHalf() { return row.scrollWidth / 2; }
  let initted = false;
  function ensureInit() {
    if (initted) return;
    if (pxPerSec < 0) row.scrollLeft = getHalf();
    initted = true;
  }
  // Init after images have started loading so scrollWidth is correct.
  // requestAnimationFrame x2 = a couple of layout passes in.
  ...
  function tick(now) {
    const dt = Math.min(0.1, (now - lastTime) / 1000);  // cap dt at 100ms to avoid huge jumps after tab inactive
    lastTime = now;
    if (initted && now >= pauseUntil) {
      const half = getHalf();
      if (half > 0) {
        row.scrollLeft += pxPerSec * dt;
        // Modulo-style wrap: keep scrollLeft within [0, half).
  ...
  // Touch + pointer interactions pause the auto-scroll so the user can
  // swipe freely. Resumes 2s after the gesture ends.
  function pauseForever() { pauseUntil = Infinity; }
  function resumeSoon()   { pauseUntil = performance.now() + 2000; }

  row.addEventListener('touchstart',  pauseForever, { passive: true });
  row.addEventListener('touchend',    resumeSoon,   { passive: true });
  row.addEventListener('touchcancel', resumeSoon,   { passive: true });
  // Also for mouse on hover (desktop) — though desktop doesn't init the
  // marquee anyway. Harmless on touch-only devices.
  row.addEventListener('mouseenter',  pauseForever);
