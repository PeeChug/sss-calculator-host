...
function renderProjectTypeCards() {
  ...
  __doc.querySelectorAll('#projectTypeCards .selectable-card').forEach(card => {
    card.addEventListener('click', () => {
      ...
      // Clear any previous _seq if the type just changed — a project that
      // switches from Fence to Deck shouldn't keep its old type's sequence.
      // Then assign a fresh one for the new type.
...
function computeDIYComparison(proTotal) {
  ...
  projects.forEach(p => {
    ...
    totalStainCost += stainCostForProj;
    ...
    // Per-project total = stain + tools + time + citronella (sprayer is
    // amortized across all projects so it isn't included here).
    const projectTotal = stainCostForProj + toolsCost + (hours * 25) + citronellaCostForProj;
    projectLines.push({
      label: `${PROJECT_META[p.type].icon} ${PROJECT_META[p.type].name} — ${pailsLabel} × $${pailCost} stain, $${toolsCost} tools, ${hours} hrs @ $25/hr${citronellaCostForProj > 0 ? `, +$${citronellaCostForProj} citronella` : ''}`,
      total: projectTotal
    });
  });
  ...
  const diyTotal = totalStainCost + totalCitronellaCost + sprayerCost + totalToolsCost + timeCost;
