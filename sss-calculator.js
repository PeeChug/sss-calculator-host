...
const DISCOUNTS = [
  ...
  { id: 'sw_referral',    label: 'Sherwin-Williams Referral',     sub: 'Referred by their Sherwin-Williams paint rep. Auto-applied because this quote is flagged SW-referred on Step 1 — flip the toggle there to remove it.', rate: 0.10, swOnly: true,
...
function getColorLibrary(productType, tier) {
  ...
  if (productType === 'oil') {
    ...
    if (tier === 'performance') return isSwMode() ? 'sw_superdeck_water' : 'expert_stain_seal';
    if (tier === 'showcase')    return isSwMode() ? 'sw_superdeck_water' : 'expert_log_timber';
  }
...
const PRODUCT_FAMILY_META_OIL_SW = {
  ...
  cons: [
    'Stock colors are limited — Semi-Solid (Performance) tier opens up SW custom tinting',
...
function isSwMode() { return !!(state && state.swReferral); }
...
function applySwMode() {
  try {
    ...
    if (typeof PRODUCT_FAMILY_META !== 'undefined' && PRODUCT_FAMILY_META.oil) {
      ...
      PRODUCT_FAMILY_META.oil = isSwMode() ? PRODUCT_FAMILY_META_OIL_SW : __pfmOilStd;
    }
  } catch (e) {}
  // Reflect on the Step 1 toggle row.
  try { const t = __doc.getElementById('swReferralToggle'); if (t) t.classList.toggle('checked', isSwMode()); } catch (e) {}
}
...
function setSwReferralDiscounts(on) {
  const apply = (p) => {
    if (!p) return;
    p.selectedDiscounts = p.selectedDiscounts || [];
    const i = p.selectedDiscounts.indexOf('sw_referral');
    if (on && i === -1) p.selectedDiscounts.push('sw_referral');
    if (!on && i >= 0) p.selectedDiscounts.splice(i, 1);
    delete p._cached;   // force totals recompute with the new discount set
  };
  apply(state.activeProject);
  (state.bundledProjects || []).forEach(apply);
}

function toggleSwReferral() { setSwReferral(!isSwMode()); }
...
function setSwReferral(on) {
  ...
  const clearColor = (p) => {
    if (!p) return;
    if (p.productType === 'oil' && p.tier !== 'essential' && p.selectedColor) p.selectedColor = null;
    if (p.savedColors) { delete p.savedColors.expert_stain_seal; delete p.savedColors.expert_log_timber; }
  };
  clearColor(state.activeProject);
  (state.bundledProjects || []).forEach(clearColor);
  try { if (typeof refreshAllProjectCaches === 'function') refreshAllProjectCaches(); } catch (e) {}
...
(function bootstrapDashboard() {
  ...
  try { checkAuthAndGate(); } catch (e) { console.warn('[SSS Auth] gate check failed:', e); }
  ...
  try { applyPricingOverrides().then(() => { try { renderDashboard(); } catch (e) {} }); }
  catch (e) { /* non-fatal */ }
  // Kick off initial cloud load (renderDashboard handles the loading skeleton).
  try { renderDashboard(); } catch (e) { /* renderDashboard not defined yet — wired up via show */ }
  // Resolve the Jobber connection state for the header pill on load.
  try { refreshJobberPill(); } catch (e) { /* non-fatal */ }
  // Wire the Step 1 customer type-ahead search.
  try { attachCustomerSearchListeners(); } catch (e) { /* non-fatal */ }
})();
...
function renderAddons() {
  ...
  if ((state.activeProject.tier === 'essential' || isSwMode()) && state.activeProject.addons) {
    delete state.activeProject.addons.citronella;
  }
...
function skelHtml(rows, tall) {
  ...
  for (let i = 0; i < (rows || 3); i++) {
    h += '<div class="skel' + (tall ? ' tall' : '') + '" style="width:' + (100 - (i * 13) % 45) + '%"></div>';
  }
  return h + '</div>';
}
...
function switchDashTab(name) {
  ...
  if (name === 'leads') {
    // Pop the three folders open — the tab itself is the "collapse",
    // no point making reps click each folder open again.
    ['custSubsPanel', 'custDraftsPanel', 'custAbandonedPanel'].forEach(id => {
      const p = __doc.getElementById(id);
      if (p && p.style.display !== 'none') p.open = true;
    });
    updateLeadsTabCount();
  }
...
function updateLeadsTabCount() {
  const n = (__custSubsCache || []).length + (__custDraftsCache || []).length + (__custAbandonedCache || []).length;
  ...
  if (dbn) { dbn.textContent = String(n); dbn.style.display = n > 0 ? '' : 'none'; }
...
async function loadDashAnalytics(force) {
  ...
  try {
    if (typeof __sssBridge === 'undefined' || !__sssBridge.call) {
      body.innerHTML = '<div class="cust-analytics-error">Bridge not ready — refresh the page.</div>';
      return;
    }
...
function renderDashAnalytics(stats) {
  ...
  // --- KPI cards ---
  const kpis = [
    { num: t.week  != null ? t.week  : '—', lbl: 'This week',  sub: fmtMoneyShort(m.weekTotal)  + ' quoted' },
    { num: t.month != null ? t.month : '—', lbl: 'This month', sub: fmtMoneyShort(m.monthTotal) + ' quoted' },
    { num: t.allTime != null ? t.allTime : '—', lbl: 'Last 90 days', sub: fmtMoneyShort(m.allTimeTotal) + ' quoted' },
    { num: fmtMoneyShort(m.avgEstimate), lbl: 'Avg estimate', sub: '&nbsp;' },
    { num: (stats.jobberSuccessRate != null ? stats.jobberSuccessRate + '%' : '—'), lbl: 'Jobber success', sub: (t.jobberFailed || 0) + ' need review' },
    { num: (stats.callbackRate != null ? stats.callbackRate + '%' : '—'), lbl: 'Callback requests', sub: (t.callbacks || 0) + ' total' }
  ];
  const kpiHtml = '<div class="dan-kpis">' + kpis.map(k =>
    `<div class="dan-kpi"><div class="num">${k.num}</div><div class="lbl">${k.lbl}</div><div class="sub">${k.sub}</div></div>`).join('') + '</div>';

  // --- Source split (SW vs regular) — v2 backend field ---
  let splitHtml = '';
  if (stats.bySource) {
    const sw = stats.bySource.sw || {}, reg = stats.bySource.regular || {};
    splitHtml = `
  ...
  const mixMax = mixEntries.length ? Math.max(...mixEntries.map(e => e[1])) : 1;
  ...
  if (stats.funnel && stats.funnel.byStage) {
    const STAGE_NAMES = { 1: 'Contact info', 2: 'Project type', 3: 'Measurements', 4: 'Condition', 5: 'Product', 6: 'Tier', 7: 'Color', 8: 'Add-ons', 9: 'Discounts', 10: 'Review' };
    ...
    const fMax = Math.max(1, ...rows.map(r => r.c), Number(stats.funnel.submitted) || 0);
    funnelHtml = `
      <div class="dan-section">
        <h4>Abandoned drafts — how far they got</h4>
        ${rows.length === 0 ? '<div class="dan-note">No abandoned drafts right now — nice.</div>' : rows.map(r => `
  ...
  const v2Missing = (!stats.bySource && !stats.weekly && !stats.funnel)
  ...
  const zeroNote = (t.allTime || 0) === 0
    ? '<div class="dan-note" style="background:var(--gold-pale);border:1px solid var(--gold);border-radius:8px;padding:10px 12px;color:#5a4a1f;font-style:normal;margin-bottom:14px;"><strong>Why zeros?</strong> These KPIs count <strong>completed submissions</strong> from the public calculators — there are none in the last 90 days yet. Abandoned drafts don\'t count as submissions; they show in the funnel ' + (stats.funnel ? 'below.' : 'once <code>customerSubmit.jsw</code> is re-published in Velo.') + '</div>'
    : '';
...
async function loadCustomerSubmissions() {
  ...
  try {
    ...
    const res = await __sssBridge.call('listCustomerSubmissions', { limit: 100 });
    if (!res || !res.ok) {
    ...
    subsPanel.style.display = __custSubsCache.length === 0 ? 'none' : 'block';
    ...
    if (draftsCount) draftsCount.textContent = String(__custDraftsCache.length);
    if (draftsPanel) {
      draftsPanel.style.display = __custDraftsCache.length === 0 ? 'none' : 'block';
    ...
    try { updateLeadsTabCount(); } catch (e2) {}
  } catch (e) {
...
function openLeadMenu(ev, rowId) {
  ...
  if (!it) return;
  const items = [];
  items.push({ label: '📋 Details', fn: () => openSubmissionDetails(rowId) });
  if (it.jobberWebUri) items.push({ label: '↗ Open in Jobber', fn: () => window.open(it.jobberWebUri, '_blank', 'noopener') });
  if (it.clientHubUri) items.push({ label: '👤 Client Hub', fn: () => window.open(it.clientHubUri, '_blank', 'noopener') });
  items.push('---');
  items.push({ label: '🗑 Delete', danger: true, fn: () => dismissCustomerSubmission(rowId, (it.customer && it.customer.name) || '') });
...
function openAbandonedMenu(ev, reference) {
  ...
  const c = it.customer || {};
  const items = [];
  items.push({ label: '📋 Details', fn: () => openDraftDetails(reference) });
  if (c.phone) items.push({ label: '📞 Call',  fn: () => { try { window.location.href = 'tel:' + String(c.phone).replace(/[^0-9+]/g, ''); } catch (e) {} } });
  if (c.email) items.push({ label: '✉ Email', fn: () => { try { window.location.href = 'mailto:' + c.email + '?subject=' + encodeURIComponent('Your Superior Stain Solutions estimate (' + (it.reference || '') + ')'); } catch (e) {} } });
  items.push('---');
  items.push({ label: 'Dismiss draft', danger: true, fn: () => dismissAbandonedDraft(reference) });
  __leadMenuShow(ev, items);
}
...
function __leadMenuShow(ev, items) {
  ...
  menu.style.left = Math.max(8, Math.min(rect.right - 180, window.innerWidth - 208)) + 'px';
  __doc.appendChild(menu);
  __openRowMenu = menu;
}
...
function wireLeadSwipe(body) {
  ...
  let card = null, inner = null, startX = 0, startY = 0, baseX = 0, mode = null;
  ...
  body.addEventListener('touchend', () => {
    if (!inner || mode !== 'swipe') { inner = null; card = null; return; }
    ...
    setTimeout(() => { try { innerRef.style.transition = ''; } catch (e) {} }, 220);
    card._swipeOpen = open;
    inner = null; card = null;
  }, { passive: true });
}
...
function buildCustSubsCardHtml(it) {
  ...
  const projSummary = projTypes.length === 0 ? '' : projTypes.slice(0, 3).join(' · ') + (projTypes.length > 3 ? ` +${projTypes.length - 3}` : '');
  const isCallback = it.submitMode === 'callback';
  const jobberFailed = !it.jobberOk && it.jobberError;
  const tags = [
    isCallback     ? `<span class="cust-subs-card-tag callback">Callback</span>` : '',
    jobberFailed   ? `<span class="cust-subs-card-tag failed">Jobber failed</span>` : ''
  ].filter(Boolean).join(' ');
  const metaBits = [
    tags,
  ...
  const totalLine = it.finalTotal > 0 ? `<div class="cust-subs-card-total">${fmtMoney(it.finalTotal)}</div>` : '';
  ...
  const metaLine = [metaBits, c.address ? `<span>${escapeHtml(c.address)}</span>` : ''].filter(Boolean).join('<span class="sep"> · </span>');
  return `
...
async function dismissCustomerSubmission(rowId, customerName) {
  if (!rowId) return;
  const label = customerName && customerName.trim()
    ? `${customerName.trim()}'s submission`
    : 'this submission';
  if (!confirm(`Permanently remove ${label} from the dashboard?\n\nThe Jobber Request + Quote will stay in Jobber — only the local dashboard row is deleted.`)) return;
  try {
    const res = await __sssBridge.call('deleteCustomerSubmission', { rowId });
    if (!res || !res.ok) {
    ...
    const subsPanel = __doc.getElementById('custSubsPanel');
    const draftsPanel = __doc.getElementById('custDraftsPanel');
    if (subsPanel   && __custSubsCache.length   === 0) subsPanel.style.display   = 'none';
    if (draftsPanel && __custDraftsCache.length === 0) draftsPanel.style.display = 'none';
    try { updateLeadsTabCount(); } catch (e2) {}
  } catch (e) {
...
function estimateDraftProjectTiers(p) {
  ...
  if (!p || !p.type || typeof PRICING === 'undefined' || !PRICING[p.type]) return out;
  const saved = state.activeProject;
  try {
    ['essential', 'performance', 'showcase'].forEach(t => {
      state.activeProject = Object.assign({}, p, { tier: t, tierConfirmed: true });
      const tierBase = computeTierBase();
      const prep = computePrepCost();
      const addons = computeAddonsTotal();
      out[t] = tierBase + prep + (addons.flat || 0) + (tierBase * (addons.percentRate || 0));
    });
  } catch (e) { /* leave whatever computed */ }
  finally { state.activeProject = saved; }
  return out;
}
...
function estimateDraftRange(projects) {
  ...
  (projects || []).forEach(p => {
    ...
    if (e.showcase > 0) { lo += e.essential; hi += e.showcase; any = true; }
  });
...
function buildSubmissionDetailsHtml(it) {
  ...
  const ago = it.createdAt ? timeSince(new Date(it.createdAt)) : '';
  let dateStr = '';
  try { dateStr = it.createdAt ? new Date(it.createdAt).toLocaleString() : ''; } catch (e) {}
  ...
  const projHtml = projects.map((p, idx) => {
    ...
    if (!desc && typeof buildJobberLineItem === 'function') {
      try {
        ...
        title = title || li.name; desc = li.description;
      } catch (e) {}
    }
    if (!title) title = (PROJECT_LABELS[p.type] || p.type || ('Project ' + (idx + 1)));
  ...
  if (it._isDraft) {
    ...
    else if (draftRange)       sumLines.push('Estimated cost: ' + fmtMoney(draftRange.lo) + '–' + fmtMoney(draftRange.hi) + ' (Essential–Showcase)');
    else                       sumLines.push('Not yet priced (left before selecting measurements/package)');
...
function openSubmissionDetails(rowId) {
  ...
  const pool = [ ...(__custSubsCache || []), ...(__custDraftsCache || []) ];
  const it = pool.find(x => x && x._id === rowId);
  if (!it) { alert('Could not find that submission — try refreshing the dashboard.'); return; }
...
function renderCustomerSubmissions() {
  ...
  if (countEl) countEl.textContent = __custSubsSearch ? `${filtered.length}/${all.length}` : String(all.length);
  if (filtered.length === 0) {
...
function renderCustomerDrafts() {
  ...
  if (countEl) countEl.textContent = __custDraftsSearch ? `${filtered.length}/${all.length}` : String(all.length);
  if (filtered.length === 0) {
  ...
  body.innerHTML = `
    <div class="folder-empty" style="text-align:left; padding:6px 8px 10px; color:#a14820; font-style:italic; font-size:12px;">
      These submissions reached us but didn't push cleanly into Jobber. Open each one, check the customer's contact info, then manually create the Jobber Request + Quote.
    </div>
...
function renderCustomerAbandonedDrafts() {
  ...
  const filtered = !q ? all : all.filter(it => {
    ...
    const blob = [
      c.name, c.email, c.phone, c.address,
      it.reference, it.activeProjectType
    ].filter(Boolean).join(' ').toLowerCase();
    return blob.includes(q);
  });
  if (countEl) countEl.textContent = q ? `${filtered.length}/${all.length}` : String(all.length);
  ...
  body.innerHTML = `
    ...
    ${filtered.map((it) => {
      ...
      const projLabel = it.activeProjectType ? (PROJECT_LABELS[it.activeProjectType] || it.activeProjectType) : '';
      const stageReached = it.maxStageReached || it.currentStage || 0;
      const tags = [
        stageReached ? `<span class="cust-subs-card-tag stage">Stage ${stageReached}/10</span>` : '',
        projLabel    ? `<span class="cust-subs-card-tag stage">${escapeHtml(projLabel)}</span>` : '',
        it.bundledCount > 0 ? `<span class="cust-subs-card-tag stage">+${it.bundledCount} bundled</span>` : ''
      ].filter(Boolean).join(' ');
      const metaBits = [
        tags,
        c.email ? `<span>${escapeHtml(c.email)}</span>` : '',
      ...
      const totalLine = it.runningTotal > 0 ? `<div class="cust-subs-card-total">${fmtMoney(it.runningTotal)} <small style="color:var(--slate);font-size:11px;font-weight:500;">(in-progress)</small></div>` : '';
      const metaLine = [metaBits, c.address ? `<span>${escapeHtml(c.address)}</span>` : ''].filter(Boolean).join('<span class="sep"> · </span>');
      return `
        <div class="cust-subs-card">
...
async function dismissAbandonedDraft(reference) {
  if (!reference) return;
  if (!confirm('Remove this abandoned draft from the dashboard? You won\'t be able to see it again.')) return;
  try {
    const res = await __sssBridge.call('deleteCustomerDraft', { reference });
    if (res && res.ok) {
      __custAbandonedCache = __custAbandonedCache.filter(it => it.reference !== reference);
      renderCustomerAbandonedDrafts();
      const countEl = __doc.getElementById('custAbandonedCount');
      if (countEl) countEl.textContent = String(__custAbandonedCache.length);
      const panel = __doc.getElementById('custAbandonedPanel');
      if (panel && __custAbandonedCache.length === 0) panel.style.display = 'none';
      try { updateLeadsTabCount(); } catch (e2) {}
    } else {
      alert('Couldn\'t remove that draft: ' + ((res && res.error) || 'unknown error'));
    }
  } catch (e) {
...
function openCustCalcAnalytics() {
  ...
  if (body) body.innerHTML = '<div class="cust-analytics-loading">Loading stats…</div>';
...
async function loadCustCalcAnalytics() {
  ...
  try {
    if (typeof __sssBridge === 'undefined' || !__sssBridge.call) {
      body.innerHTML = '<div class="cust-analytics-error">Bridge not ready — please refresh.</div>';
      return;
    }
    const stats = await __sssBridge.call('getCustomerCalcStats', {});
    if (!stats || !stats.ok) {
...
function renderCustCalcAnalytics(stats) {
  ...
  const projectEntries = Object.entries(projectMix).sort((a, b) => b[1] - a[1]);
  const projectMax = projectEntries.length > 0 ? Math.max(...projectEntries.map(e => e[1])) : 1;
  const projectRows = projectEntries.map(([type, count]) => {
  ...
  }).join('') || '<div style="color:var(--slate);font-size:12px;font-style:italic;">No project data yet.</div>';

  const successRateStr = (stats.jobberSuccessRate !== null && stats.jobberSuccessRate !== undefined)
    ? stats.jobberSuccessRate + '%' : '—';
  const callbackRateStr = (stats.callbackRate !== null && stats.callbackRate !== undefined)
    ? stats.callbackRate + '%' : '—';
  ...
  body.innerHTML = `
    <div class="cust-analytics-grid">
      ...
      <div class="cust-analytics-card">
        <div class="lbl">Jobber success</div>
    ...
    <div class="cust-analytics-section" style="border-color:var(--coral); background:var(--coral-pale);">
      ...
      <div style="font-size:13px; color:var(--navy);">
        <strong>${t.jobberFailed}</strong> submission${t.jobberFailed === 1 ? '' : 's'} failed to reach Jobber and need manual processing. They're still in <strong>Customer Submissions</strong> below — click into each one to handle.
      </div>
    </div>` : ''}
  `;
}
...
async function loadDashboardData() {
  ...
  loadJobberRequests();
  // Customer Submissions folder (bottom of dashboard) — reads from the
  // CustomerSubmissions Wix collection populated by the public customer
  // calculator's submit endpoint. Hides itself when empty.
  loadCustomerSubmissions();
  // Customer Drafts (abandoned) folder — reads from CustomerDrafts,
...
function toggleDashDateFilter(which) {
  ...
  if (chip) {
    if (dashState.dateFilter) {
      chip.style.display = '';
      chip.innerHTML = '<span class="chip">Showing: quotes from ' + (dashState.dateFilter === 'week' ? 'the last 7 days' : 'the last 30 days') + ' <b onclick="toggleDashDateFilter(\'' + dashState.dateFilter + '\')" title="Clear filter">×</b></span>';
    } else { chip.style.display = 'none'; chip.innerHTML = ''; }
  }
  // Jump to the Quotes tab — that's what the filter applies to.
  if (dashState.dateFilter && typeof switchDashTab === 'function' && __dashTab !== 'quotes') switchDashTab('quotes');
  renderDashboard();
}
...
function renderFolder(folder) {
  const items = (dashState.cache[folder] || []).filter(it => matchesSearch(it) && matchesDateFilter(it));
...
function openPricingAdmin() {
  ...
  __paWorking = {
    ...
    discounts: DISCOUNTS.map(d => {
      const ov = (__pricingOverrides.discounts || []).find(p => p && p.id === d.id);
      return (ov && typeof ov.rate === 'number') ? { id: d.id, rate: ov.rate } : { id: d.id };
    }),
...
function paField(path, step) {
  ...
  const isOverridden = (typeof override === 'number') && (typeof def === 'number') && override !== def;
...
const SW_TIER_DELTAS = { deck: { showcase: 6.63 }, pergola: { showcase: 7.29 }, barn: { showcase: 5.66 }, ceiling: { showcase: 5.66 } };
...
function swDefaultsRoot() {
  ...
  Object.keys(SW_TIER_DELTAS).forEach(p => { if (r[p] && r[p].tiers) Object.assign(r[p].tiers, SW_TIER_DELTAS[p]); });
  __swDefaultsCache = r;
  return r;
}
...
function paDefaultFor(path) {
  if (path.indexOf('rulesSw.') === 0) return paWalk({ rulesSw: swDefaultsRoot() }, path);
  return paWalk({ rules: __builtInPricing, discounts: __builtInDiscounts }, path);
}
...
function renderPASw() {
  ...
  return `
    <div class="pa-section">
      ...
      <p style="font-size:12px;color:var(--slate);margin:0 0 10px;">These apply <strong>only to the Sherwin-Williams referral calculator</strong> (/customer-calculator-sw), layered on top of the shared rates. Its built-in Showcase rates run higher (deck $6.63 · pergola $7.29 · barn/ceiling $5.66). Everything else — prep, fence styles, water tiers, add-ons, discounts, DIY — follows the shared tabs.</p>
      <div class="pa-grid">
...
function paLiveVal(path) {
  const inp = __doc.querySelector(`#pricingAdminBody input[data-pa-path="${path}"]`);
  if (inp) { const v = parseFloat(inp.value); if (!isNaN(v)) return v; }
...
function updatePaTierPreview() {
  ...
  if (!el) return;
  const f = (rate, units) => '$' + Math.round(rate * units).toLocaleString();
  ...
  el.innerHTML = '<strong>Live preview — 200 ln ft privacy fence (6 ft, both sides):</strong> '
    + 'Essential oil ' + f(oilE, 200) + ' · Essential water ' + f(watE, 200)
    + ' · Performance ' + f(perf, 200) + ' · Showcase ' + f(show, 200)
    + '<br><strong>300 sq ft deck @ Performance:</strong> ' + f(deckP, 300)
...
function renderPricingAdminTab(tab) {
  ...
  if (tab === 'reps') {
    ...
    return '<div id="repsTabBody" data-ticket="' + __paAdminTicket + '">' + skelHtml(3, true) + '</div>';
  }
  if (tab === 'devices') {
    __paAdminTicket++;
    setTimeout(loadAndRenderDevices, 0);
    return '<div id="devicesTabBody" data-ticket="' + __paAdminTicket + '">' + skelHtml(3, true) + '</div>';
  }
  return '';
}
...
async function savePricingAdmin() {
  ...
  const cleanRulesSw = compactRulesOverride(__paWorking.rulesSw || {}, swDefaultsRoot()) || {};
  try {
    const res = await __sssBridge.call('savePricingRules', { rules: cleanRules, discounts: cleanDiscounts, rulesSw: cleanRulesSw });
    if (!res || !res.ok) {
      paToast('Save failed: ' + ((res && res.error) || 'unknown error'));
      return;
    }
    __pricingOverrides = { rules: cleanRules, rulesSw: cleanRulesSw, discounts: cleanDiscounts };
    ...
    for (let i = 0; i < node.length; i++) {
      ...
      if (typeof v === 'object') {
        ...
        if (sub && (Array.isArray(sub) ? sub.some(x => x != null) : Object.keys(sub).length > 0)) {
          ...
          if (!Array.isArray(sub) && defVal && defVal.id && !sub.id) sub.id = defVal.id;
