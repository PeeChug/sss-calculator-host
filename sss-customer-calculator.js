...
// Driven by the Web Animations API instead of CSS @keyframes — we
// compute the exact pixel distance to translate based on each tile's
// real rendered width, then animate to that distance over a fixed
// duration. This is reliable across every browser because the values
// are explicit pixels (no percentage interpretation), and Web
// Animations API gives us a clean `animation.pause()` / `.play()`
// API for the touch-pause behavior.
...
function setupOneMarquee(row, attempt) {
  ...
  // Sum the widths of the FIRST HALF of tiles (the originals); that
  // distance equals exactly the offset where the duplicate section
  // begins — animating from 0 to -halfWidth loops seamlessly.
  ...
  // If layout isn't ready yet (zero-width tiles because images
  // haven't loaded or fonts haven't settled), retry. Up to 30 tries
  // × 100ms = 3 seconds before giving up.
  if (halfWidth < 50) {
    if (attempt < 30) {
      setTimeout(function () { setupOneMarquee(row, attempt + 1); }, 100);
    } else {
      console.warn('[Customer] hero marquee: track width never settled for', row);
    }
    return;
  }

  const isReverse = row.getAttribute('data-marquee') === 'reverse';
  const duration  = isReverse ? 75000 : 60000;
  const fromX     = isReverse ? -halfWidth : 0;
  const toX       = isReverse ? 0 : -halfWidth;

  console.log('[Customer] hero marquee: row ready, halfWidth=' + halfWidth + 'px, direction=' + (isReverse ? 'reverse' : 'forward'));
  ...
  // Honour reduced-motion users — start paused so they see all photos
  // statically and the animation never runs.
  try {
  ...
  row.addEventListener('touchstart',  pauseRow,   { passive: true });
  ...
  row.addEventListener('mouseleave',  resumeSoon);
}
