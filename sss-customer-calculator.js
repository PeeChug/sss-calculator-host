...
// Customer "Start" button on intro → go to Step 1.
function goToStage1FromIntro() {
  showStage(1);
  // Reveal the header callback button once the customer is in the flow.
  const cbBtn = __doc.getElementById('custHeaderCallbackBtn');
  if (cbBtn) cbBtn.style.display = 'inline-flex';
  scrollAppToTop();
}

// Customer "Back" button on Step 1 → return to intro hero.
...
async function _doCustomerSubmit(mode) {
  ...
  try {
    ...
    // Success — show the customer success stage with reference info.
...
// Render the customer success stage with submission details.
function showCustomerSuccess(result, isCallback) {
  ...
  if (totalEl) {
    const t = (result && Number(result.total)) || 0;
    totalEl.textContent = t > 0 ? '$' + Math.round(t).toLocaleString() : '—';
  }
  if (isCallback) {
    if (headEl) headEl.textContent = 'Got it — we\'ll call you soon.';
    if (leadEl) leadEl.innerHTML = "We've received your request and a member of our team will reach out within <strong>1 business day</strong> at the number you provided. We'll talk through your project, answer any questions, and walk you through your estimate over the phone.";
  } else {
    if (headEl) headEl.textContent = 'Your estimate is on the way.';
    if (leadEl) leadEl.innerHTML = "We've received your request and emailed a copy of your estimate to the address you provided. A member of our team will reach out within <strong>1 business day</strong> to confirm details and schedule your free in-person measurement.";
  }
  ...
  // Hide running-total + callback pills on the success screen.
  const totalPill = __doc.getElementById('totalPill');
