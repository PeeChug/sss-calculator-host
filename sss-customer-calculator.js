      ...
      <div class="pay-opt-grid" style="margin-top:8px; display:flex; flex-direction:column; gap:8px;">
        ...
        <label class="pay-opt" style="display:flex; align-items:flex-start; gap:10px; padding:10px 12px; background:var(--paper); border:1.5px solid var(--line); border-radius:8px; cursor:pointer;">
          ...
          <div style="flex:1;">
            ...
            <div class="pay-opt-detail cust-mc" style="margin-top:6px; font-size:13px; line-height:1.5;">
              <strong style="color:var(--navy);">Deposit: $${Math.round(totals.finalTotal * 0.25).toLocaleString()}</strong> at scheduling. Wisetack covers the remaining $${Math.round(totals.finalTotal * 0.75).toLocaleString()} (≈ $${Math.round((totals.finalTotal * 0.75)/24).toLocaleString()}/mo over 24 months, soft credit pull).
              <div style="font-size:11px; margin-top:4px;">*Final rate determined by Wisetack credit check. Deposit required on every quote.</div>
            </div>
          </div>
        </label>
      </div>
