...
function renderConditionCards() {
  ...
  __doc.getElementById('conditionCards').innerHTML = order.map(key => {
    ...
    return `
      <button class="condition-card ${isReco ? 'recommended' : ''} ${isSelected ? 'selected expanded' : ''} ${isLocked ? 'locked' : ''}" data-cond="${key}" data-mobile-collapse="true" ${isLocked ? 'aria-disabled="true" tabindex="-1"' : ''}>
  ...
  __doc.querySelectorAll('#conditionCards .condition-card').forEach(card => {
    ...
    card.addEventListener('click', (e) => {
      ...
      // Selecting also auto-expands the chosen card (on mobile that
      // means the customer sees what they just picked); other cards
      // collapse back so the page doesn't stay tall.
...
function renderProductStage() {
  ...
  __doc.getElementById('productChoiceCards').innerHTML = order.map(prod => {
    ...
    return `
      <button class="product-choice-card ${isReco ? 'recommended' : ''} ${isSelected ? 'selected expanded' : ''}" data-product="${prod}" data-mobile-collapse="true">
  ...
  __doc.querySelectorAll('#productChoiceCards .product-choice-card').forEach(card => {
    ...
    card.onclick = (e) => {
      ...
      // Selection also auto-expands the chosen card on mobile so the
      // customer sees what they picked; others collapse.
      __doc.querySelectorAll('#productChoiceCards .product-choice-card').forEach(c => {
        ...
        c.classList.toggle('expanded', picked);
      });
...
function renderTierCards() {
  ...
  __doc.getElementById('tierCards').innerHTML = ['essential', 'performance', 'showcase'].map(t => {
    ...
    return `
      <button class="tier-card ${isReco ? 'recommended' : ''} ${isSelected ? 'selected expanded' : ''}" data-tier="${t}" data-mobile-collapse="true">
        ${isReco ? '<div class="reco-flag">Recommended</div>' : ''}
  ...
  __doc.querySelectorAll('#tierCards .tier-card').forEach(card => {
    card.addEventListener('click', (e) => {
      ...
      state.activeProject.tierConfirmed = true;
      // Selection auto-expands the chosen card on mobile; others collapse.
      ...
      <div class="addon-grid service-includes-grid">${serviceAddons.map(a => `
        <div class="service-include-row" data-mobile-collapse="true" onclick="toggleMobileExpand(event)" role="button" tabindex="0">
          ...
          <div class="addon-desc">
            <div class="ad-name">${a.name}${a.desc ? ' <span class="sir-chev">▾</span>' : ''}</div>
...
function toggleCustHelpMenu() {
  ...
  if (menu.classList.contains('open')) {
    // On phones the menu is `position: fixed` (CSS at the @media
    // breakpoint), so we explicitly set its `top` to sit just below
    // the help button's current viewport position. Avoids the
    // overflow-clipping issue when an absolute-positioned menu hit
    // the floating-bar boundaries on small screens.
    try {
