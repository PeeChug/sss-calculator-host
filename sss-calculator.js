...
async function pushFinishedQuoteToJobber(rowId) {
  ...
  try {
    const r = await fetch('/_functions/pushToJobber', {
      ...
      body: JSON.stringify({ quoteRowId: rowId })
    });
    const data = await r.json();
    state._lastJobberPush = data;
    if (!box) return;
    if (data && data.ok) {
      const numberLine = data.jobberQuoteNumber ? `<div style="font-size:12px;color:var(--slate);margin-top:2px;">Quote #${escapeHtml(data.jobberQuoteNumber)}</div>` : '';
      box.innerHTML = `
        <div class="jobber-push-row success">
          <span class="ico">✓</span>
          <div>
            <div><strong>Sent to Jobber</strong></div>
            ${numberLine}
          </div>
        </div>`;
    } else {
