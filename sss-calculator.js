...
async function pushFinishedQuoteToJobber(rowId, force, sendMethod) {
  ...
  if (box) {
    const verb = sendMethod === 'open' ? 'Pushing to Jobber, then opening it for you…'
               : sendMethod === 'none' ? 'Saving to Jobber as a draft…'
               : force ? 'Re-sending to Jobber…' : 'Pushing to Jobber…';
    ...
    if (data && data.ok) {
      ...
      if (Array.isArray(data.sentLineItems) && data.sentLineItems.length) {
        ...
        if (data.sendMethod === 'open') {
          sendMethodPill = pill('#e8f5ff', '#0b69b8', '✓ Opened in Jobber — hit Send there');
        } else if (data.sendMethod === 'none') {
          sendMethodPill = pill('var(--line-soft)', 'var(--slate)', 'Saved as draft');
        } else {
          sendMethodPill = pill('var(--line-soft)', 'var(--slate)', 'Ready in Jobber');
        }
...
function buildJobberLineItem(p, idx, total) {
  ...
  const lines = [];
  // Helper: visual section divider for grouping content in the line
  // item description. Plain text since Jobber's PDF rendering of line
  // item descriptions is not Markdown-aware.
  const SECTION = '─────────────';
  const pushSection = (heading) => { lines.push(''); lines.push(`${SECTION}`); lines.push(heading); lines.push(`${SECTION}`); };
