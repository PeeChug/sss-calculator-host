...
async function loadDashAnalytics(force) {
  ...
  if (!body) return;
  if (force) body.innerHTML = '<div class="cust-analytics-loading">Refreshing…</div>';
  try {
...
function buildCustSubsCardHtml(it) {
  ...
  const jobberBtn = it.jobberWebUri
    ? `<a class="btn btn-secondary" href="${escapeHtml(it.jobberWebUri)}" target="_blank" rel="noopener">↗ Open in Jobber</a>`
    : '';
  const hubBtn = it.clientHubUri
    ? `<a class="btn btn-secondary" href="${escapeHtml(it.clientHubUri)}" target="_blank" rel="noopener">👤 Client Hub</a>`
    : '';
  // Hard-delete button — removes the row from the CustomerSubmissions
  // collection. Jobber record stays (operational source of truth).
  // Customer name is escaped + JSON-string-encoded so single quotes
  // in names don't break the inline onclick string.
  const nameEsc = JSON.stringify((it.customer && it.customer.name) || '');
  const deleteBtn = it._id
    ? `<button type="button" class="btn-dismiss" onclick="dismissCustomerSubmission('${escapeHtml(it._id)}', ${escapeHtml(nameEsc)})" title="Remove this submission from the dashboard (Jobber record stays)">🗑 Delete</button>`
    : '';
  // 📋 Details — opens a modal with the full stage-by-stage breakdown of
  // what the customer selected (tier, stain, color, measurements, prep,
  // add-ons, discounts, etc.). Rendered entirely from cached row data —
  // no extra backend call.
  const detailsBtn = it._id
    ? `<button type="button" class="btn btn-secondary" onclick="openSubmissionDetails('${escapeHtml(it._id)}')" title="See everything this customer selected">📋 Details</button>`
    : '';
  return `
    <div class="cust-subs-card">
      <div class="cust-subs-card-main">
        ...
        ${c.address ? `<div class="req-card-title">${escapeHtml(c.address)}</div>` : ''}
        ${metaBits ? `<div class="cust-subs-card-meta">${metaBits}</div>` : ''}
        ${totalLine}
      </div>
