// sss-customer-calculator-sw.js — auto-generated from customer-calculator-sw.html
// =============================================================
// Renders the SHERWIN-WILLIAMS edition of the Superior Stain
// Solutions estimate calculator as a Custom Element
// (<sss-customer-calculator-sw>) inside a Shadow DOM.
//
// Same submission backend as the standard customer build —
// posts to /_functions/submitCustomerEstimate which creates the
// Jobber Request + Draft Quote. The product lineup, discounts,
// and eyebrow copy differ from the standard build; everything
// else (stages, validation, submission flow) is identical.
//
// To rebuild after editing customer-calculator-sw.html, run:
//   python3 build-customer-element-sw.py
// =============================================================
(function () {
  if (window.customElements && customElements.get('sss-customer-calculator-sw')) return;
...
const PRICING = {
  ...
  deck:  { tiers: { essential: 4.00, performance: 5.00, showcase: 6.63 }, rates: { flat: 5.00, railing: 7.50, stair: 31.25, lattice: 3.75 }, underneathMultiplier: 2, prep: { no_wash: 0, soft_wash: 1.25, strip_sand: 2.85 }, unit: 'sq ft' },
  pergola: { tiers: { essential: 4.40, performance: 5.50, showcase: 7.29 }, overheadAccessFlat: 200, prep: { no_wash: 0, soft_wash: 1.25, strip_sand: 2.85 }, unit: 'sq ft' },
  ...
  // is the same.
  barn:    { tiers: { essential: 3.40, performance: 4.25, showcase: 5.66 }, heightPremium: 1.30, liftRentalPerDay: 400, trimRate: 1.50, cupolaFlat: 200, prep: { no_wash: 0, soft_wash: 1.25, strip_sand: 2.85 }, unit: 'sq ft' },
  ceiling: { tiers: { essential: 3.40, performance: 4.25, showcase: 5.66 }, tngPremium: 0.50, beamRate: 8.00, fixtureRemoval: 50, fanRemoval: 100, furnitureProtFlat: 100, prep: { no_wash: 0, soft_wash: 1.25, strip_sand: 2.85 }, unit: 'sq ft' },
  stainUpgrades: [
    ...
    { id: 'custom_color',name: 'Custom color match (you provide a sample or code)', priceType: 'flat', rate: 0,
      img: 'https://static.wixstatic.com/media/6616da_a36a623288334b47bcc281830a52fa1f~mv2.jpg/v1/fit/w_900,h_900,q_80,enc_auto/file.jpg',
...
// SW Edition: only the Sherwin-Williams referral discount is shown
// on the discounts page. Auto-applied for any customer using this
// version of the calculator (a Sherwin-Williams paint rep sent them).
// Bundle savings still apply automatically when 2+ projects are added,
...
// SW version keeps the cap at 10% — the SW referral covers the full cap;
// bundle savings (also 10%) stack separately at the multi-project level.
...
/* ============================================================
   ...
   ============================================================ */
const COLORS = {
  // SuperDeck Oil-Based Transparent — Essential oil tier (5 stock colors)
  sw_superdeck_transparent: {
    line: 'SuperDeck Oil-Based Transparent',
    note: 'Penetrating oil-based transparent stain. Lightest pigment load — natural wood grain stays visible. Hex swatches are approximations — actual color varies with your wood species, age, and lighting.',
    grouped: true,
    groups: [
      { label: 'Natural wood tones', colors: [
        { name: 'Natural',           code: 'SW', hex: '#c8a984' },
        { name: 'Cedar Tone Natural',code: 'SW', hex: '#b88c64' },
        { name: 'Redwood',           code: 'SW', hex: '#9c5530' },
        { name: 'Heart Redwood',     code: 'SW', hex: '#7a3a22' },
        { name: 'Canyon Brown',      code: 'SW', hex: '#6e4530' }
      ]}
    ]
  },
  // SuperDeck Exotic Timber Oil — Performance oil tier (4 stock colors)
...
function getTierMeta(productType, tier, projectType) {
  ...
  const pt = projectType || (typeof state !== 'undefined' && state.activeProject && state.activeProject.type);
...
const PRODUCT_FAMILY_META = {
  water: {
    icon: '💧',
    heading: 'Water-Based',
    img: 'https://static.wixstatic.com/media/6616da_8c8ed8795f6f4bc2a30ccd57e77d9a22~mv2.jpg/v1/fit/w_900,h_900,q_80,enc_auto/file.jpg',
  ...
  oil: {
    icon: '🛢️',
    heading: 'Oil-Based',
    img: 'https://static.wixstatic.com/media/6616da_5cead61260e74114831ef77b95c9d217~mv2.jpg/v1/fit/w_900,h_900,q_80,enc_auto/file.jpg',
    ...
    pros: [
      ...
      'Trans-oxide pigments deliver long-lasting UV defense',
      'Water-repellent and mold/mildew-inhibiting agents built in',
      'Stronger UV resistance per coat — built for southern sun',
      'Exotic Timber Oil tri-oil blend (tung + linseed + alkyd) works on every exterior wood species',
      'Better for high-exposure decks, pergolas, and exotic hardwoods'
    ],
    cons: [
      'Stock colors are limited — Semi-Solid (Showcase) tier opens up SW custom tinting',
  ...
  hoa: {
    ...
    recommendNote: 'Pick this only if your HOA mandates a specific brand or color outside our standard SW SuperDeck / Woodscapes lines.'
  }
};
...
//
// Woodscapes (the standard water lineup below) is a VERTICAL-surface
// house stain: siding, fences, log walls. It's film-forming and carries
// no deck warranty, so on horizontal wood that holds standing water and
// takes foot traffic it peels. Decks get the SuperDeck waterborne line,
// which Sherwin-Williams formulates specifically for horizontal wood.
// Resolved per-project by getTierMeta(); fences, barns, pergolas, and
// porch ceilings keep Woodscapes.
//
...
const TIER_META_WATER_DECK = {
  ...
  essential: {
    ...
    bestFor: 'Newer decks in good shape where you want the grain to show'
  },
  performance: {
...
// Decks only — fences, barns, pergolas, and porch ceilings are
// vertical or protected, where Woodscapes is the correct product.
...
const TIER_META = {
  water: {
    ...
    essential: {
      ...
      badge: 'Budget Friendly',
      badgeClass: 'flag-green',
      tagline: 'Premium solid-color value',
      life: '4–5 years',
      details: 'Sherwin-Williams Woodscapes Solid Color Exterior Stain — a premium water-based solid color stain with strong UV resistance and rich film build. Soap-and-water cleanup, low VOC, fast same-day turnaround in good weather. Coverage ~150 sq ft per gallon.',
      pros: ['Best water-based value', 'Even color, no streaks', 'Strong UV fade resistance', 'Easy water cleanup', 'Low VOC'],
      cons: ['Solid color only (no transparency)', 'Less self-cleaning than the Performance tier'],
      bestFor: 'Most fences and decks — strong protection at the best water-based price'
    },
    performance: {
      product: 'SW Woodscapes Rain Refresh',
      badge: 'Most Popular',
      badgeClass: 'flag-blue',
      tagline: 'Self-cleaning — our most-recommended',
      life: '5–7 years',
      details: 'SW Woodscapes Rain Refresh — a solid-color exterior stain with Self-Cleaning Technology: rainfall lifts surface dirt off so the finish stays looking new. Carries a 10-year limited manufacturer warranty and includes a free 30-day touch-up visit.',
      pros: ['Self-cleaning surface (SW Self-Cleaning Technology)', '10-year limited manufacturer warranty', 'Strong UV + moisture defense', '30-day touch-up visit included'],
      cons: ['Higher up-front cost than Essential', 'Solid color only (no transparency)'],
      bestFor: 'Most homeowners — full-sun decks, privacy fences, low-maintenance finish'
    },
    showcase: {
      product: 'SW Acrylic Alkyd Stain',
      badge: 'Longest-Lasting',
      badgeClass: 'flag-gold',
      tagline: 'Acrylic-alkyd hybrid — maximum durability',
      life: '7+ years',
      details: 'Sherwin-Williams SuperDeck Acrylic-Alkyd Solid Color Stain — SW\'s most durable exterior wood coating, carrying up to a 15-year manufacturer warranty on fences with proper prep. Full opaque solid color with the heaviest film build and pigment loading in the lineup.',
...
/* ============================================================
   ...
   ============================================================ */
const PROJECT_META = {
  fence:   { name: 'Fence',          icon: '🪵', unit: 'ln ft', img: 'https://static.wixstatic.com/media/6616da_70b75370c79a48b39d20cdb5c99c5323~mv2.jpg/v1/fit/w_900,h_900,q_80,enc_auto/file.jpg', desc: 'Linear feet × height. Both sides standard. Privacy, shadowbox, board-on-board, farm fence.', badge: 'Most common' },
  deck:    { name: 'Deck',           icon: '🌳', unit: 'sq ft', img: 'https://static.wixstatic.com/media/6616da_3dac2dabdd894c3abcf3491e2b954996~mv2.jpg/v1/fit/w_900,h_900,q_80,enc_auto/file.jpg', desc: 'Flat surface sq ft + railings + stairs. Optional underneath staining and lattice walls.', badge: 'Most common' },
  pergola: { name: 'Pergola',        icon: '⛱️', unit: 'sq ft', img: 'https://static.wixstatic.com/media/6616da_ab2a8aeff2ec435b8f8385c1c0454c91~mv2.jpg/v1/fit/w_900,h_900,q_80,enc_auto/file.jpg', desc: 'Total surface sq ft — top + bottom of beams + posts. Plant tarping included.' },
  ceiling: { name: 'Wooden Ceiling', icon: '🏠', unit: 'sq ft', img: 'https://static.wixstatic.com/media/6616da_36be1e29989c4a349547c1bf70ed16ec~mv2.jpg/v1/fit/w_900,h_900,q_80,enc_auto/file.jpg', desc: 'Porch ceilings, exposed beams, T&G. Easier than exterior — protected from weather.' },
...
(function bootstrapCustomer() {
  ...
  // Fire up the hero photo-gallery marquees on mobile (two rows
  // continuously scroll in opposite directions; touch pauses each row
...
// =============================================================
// HERO GALLERY MARQUEES (mobile only)
// =============================================================
// CSS handles the smooth GPU-accelerated drift via @keyframes
// translateX. This JS only adds the touch-pause behavior so the user
// can manually swipe the row while the animation is paused — then
// resume the animation ~1.5s after they let go.
//
// The row is `overflow-x: auto` so native swipe always works. While
// the .paused class is set, the CSS animation is frozen
// (animation-play-state: paused), so the track stays put and the
// user's scroll input is the only thing moving the visual.
// Hero gallery marquee — fire-and-forget. Drives a Web Animations API
// translate3d animation on each .cust-hero-gallery-track. No touch,
// no hover, no click, no pause — just continuous scrolling. Distance
// is calculated from actual tile widths so it works the same on
// every browser/viewport. Retries up to 3s if layout isn't ready.
function startHeroMarquees() {
  if (typeof window === 'undefined') return;
  fadeInHeroImages();
  const rows = __doc.querySelectorAll('.cust-hero-gallery-row[data-marquee]');
  console.log('[Customer] hero marquee: starting (' + rows.length + ' rows, viewport ' + window.innerWidth + 'px)');
  rows.forEach(function (row) { setupOneMarquee(row, 0); });
}

// Reveal each hero photo only once it's fully decoded. Prevents the
// "loads a portion, leaves a white bar" look on slow connections — the
// tile shows its gray placeholder until the image is 100% ready, then
// fades in. Already-cached images show instantly (no fade). Broken
// images are revealed too (error handler) so nothing stays hidden.
function fadeInHeroImages() {
  const imgs = __doc.querySelectorAll('.cust-hero-gallery-tile img');
  imgs.forEach(function (img) {
    ...
    const reveal = function () { img.classList.add('img-loaded'); };
    img.addEventListener('load', reveal, { once: true });
    img.addEventListener('error', reveal, { once: true });
  });
}

function setupOneMarquee(row, attempt) {
  attempt = attempt || 0;
  const track = row.querySelector('.cust-hero-gallery-track');
  if (!track) return;
  const tiles = Array.from(track.children);
  if (tiles.length < 2) return;

  // Sum the widths of the FIRST HALF of tiles (the originals). The
  // marquee animates from 0 to -halfWidth — at -halfWidth the
  // duplicate section is in the visible position, identical to the
  // original at 0, so the loop is seamless.
  const halfCount = Math.floor(tiles.length / 2);
  let halfWidth = 0;
  for (let i = 0; i < halfCount; i++) {
    const t = tiles[i];
    const m = parseFloat(getComputedStyle(t).marginRight) || 0;
    halfWidth += t.offsetWidth + m;
  }
  // Retry if layout isn't ready (images / fonts still settling).
  if (halfWidth < 50) {
    if (attempt < 30) setTimeout(function () { setupOneMarquee(row, attempt + 1); }, 100);
    else console.warn('[Customer] hero marquee: track width never settled for', row);
    return;
  }

  const isReverse = row.getAttribute('data-marquee') === 'reverse';
  // Adaptive duration: aim for ~23 px/sec on the forward row and
  // ~19 px/sec on the reverse row (slightly different so they don't
  // feel mechanically synchronized). Computing from halfWidth means
  // adding/removing photos doesn't change the perceived speed.
  const targetPxPerSec = isReverse ? 19 : 23;
  const duration  = Math.round((halfWidth / targetPxPerSec) * 1000);
  const fromX     = isReverse ? -halfWidth : 0;
  const toX       = isReverse ? 0 : -halfWidth;

  console.log('[Customer] hero marquee: row ready, halfWidth=' + halfWidth + 'px, direction=' + (isReverse ? 'reverse' : 'forward') + ', duration=' + (duration / 1000) + 's');
  ...
  try {
    ...
    console.warn('[Customer] hero marquee: Web Animations API failed', e);
  }
}
...
// Cookie-based fallback channel — survives Wix preview iframe contexts
// where localStorage is siloed per session. We persist to BOTH channels
// on save and prefer localStorage on load (faster, no size limit), then
// fall back to the cookie if localStorage returned null. Cookie is
// stored on the topmost reachable domain so it survives across pages.
const CUST_PROGRESS_COOKIE = 'sss_cust_progress';
function __custWriteCookie(name, value, days) {
  try {
    const exp = new Date(Date.now() + days * 86400000).toUTCString();
    // Try to write at the eTLD+1 by stripping subdomains; falls back to
    // the current host if document.domain can't widen. SameSite=Lax to
    // survive top-level navigation.
    const host = (location.hostname || '').split('.').slice(-2).join('.');
    const domainAttr = host && host.indexOf('.') !== -1 ? `; domain=.${host}` : '';
    document.cookie = `${name}=${encodeURIComponent(value)}; expires=${exp}; path=/; SameSite=Lax${domainAttr}`;
  } catch (e) {}
}
function __custReadCookie(name) {
  try {
...
function __custDeleteCookie(name) {
  try {
    const host = (location.hostname || '').split('.').slice(-2).join('.');
    const domainAttr = host && host.indexOf('.') !== -1 ? `; domain=.${host}` : '';
    document.cookie = `${name}=; expires=Thu, 01 Jan 1970 00:00:00 GMT; path=/; SameSite=Lax${domainAttr}`;
...
function __custSaveProgress() {
  try {
    ...
    let lsOk = false, ssOk = false, ckOk = false;
    try { localStorage.setItem(CUST_PROGRESS_KEY, json); lsOk = true; } catch (e) {}
    try { sessionStorage.setItem(CUST_PROGRESS_KEY, json); ssOk = true; } catch (e) {}
    // Cookie is only used as a small fallback — bundle/customer/notes
    // can balloon over 4KB. Drop the big arrays from the cookie copy.
    try {
      const lite = { v: 1, savedAt: snap.savedAt, quoteId: snap.quoteId, currentStage: snap.currentStage, maxStageReached: snap.maxStageReached, customer: snap.customer, activeProject: { type: snap.activeProject && snap.activeProject.type } };
      const liteJson = JSON.stringify(lite);
      if (liteJson.length < 3800) { __custWriteCookie(CUST_PROGRESS_COOKIE, liteJson, 14); ckOk = true; }
    } catch (e) {}
    console.log('[CustResume] saved snapshot — stage:', snap.currentStage, 'maxReached:', snap.maxStageReached, 'name:', snap.customer && snap.customer.name, 'channels:', { localStorage: lsOk, sessionStorage: ssOk, cookie: ckOk });
  } catch (e) {
...
function __custLoadProgress() {
  // Tri-channel load — prefer the richest source first. localStorage
  // wins when present (has the full snapshot). sessionStorage survives
  // some same-session refreshes. Cookie is the last-resort fallback
  // (lighter payload, but survives Wix preview iframe storage isolation).
  const tryParse = function (raw, source) {
    if (!raw) return null;
    try {
      const snap = JSON.parse(raw);
      if (!snap || snap.v !== 1) return null;
      if (!snap.savedAt || (Date.now() - snap.savedAt) > CUST_PROGRESS_TTL_MS) {
        // Stale — silently drop from THIS channel; don't disturb others.
        return null;
      }
      snap._source = source;
      return snap;
  ...
  try {
    const ls = tryParse(localStorage.getItem(CUST_PROGRESS_KEY), 'localStorage');
    if (ls) return ls;
  } catch (e) {}
  try {
    const ss = tryParse(sessionStorage.getItem(CUST_PROGRESS_KEY), 'sessionStorage');
    if (ss) return ss;
  } catch (e) {}
  try {
    const ck = tryParse(__custReadCookie(CUST_PROGRESS_COOKIE), 'cookie');
    if (ck) return ck;
  } catch (e) {}
  return null;
}
...
function __custClearProgress() {
  ...
  try { sessionStorage.removeItem(CUST_PROGRESS_KEY); } catch (e) {}
  try { __custDeleteCookie(CUST_PROGRESS_COOKIE); } catch (e) {}
}
...
function setupFenceHeight() {
  const wrap  = __doc.getElementById('m_heightSelect');
  const input = __doc.getElementById('m_height');
  if (!wrap || !input) return;
  const m = state.activeProject.measurements;
  let h = Number(m.height) || 0;
  if (!h) { h = 6; m.height = 6; }                 // default to 6 ft
  const isPreset = (h === 6 || h === 8);
  const mark = (mode) => wrap.querySelectorAll('.height-opt')
    .forEach(b => b.classList.toggle('selected', b.dataset.height === mode));
  input.value = h;
  if (isPreset) { input.style.display = 'none'; mark(String(h)); }
  else          { input.style.display = '';      mark('other'); }
...
function renderProductStage() {
  ...
  __doc.getElementById('productChoiceCards').innerHTML = order.map(prod => {
    ...
    const famSummary = (prod === 'water' && usesDeckWaterLine(state.activeProject.type))
...
function renderTierCards() {
  ...
  const proj = state.activeProject.type;
  // Use getTierMeta() per-tier so the SW Edition's fence override
  // (Woodscapes oil ladder) is honored. Falling back to TIER_META[product]
  // directly would always show SuperDeck even on fence projects.
  ...
  // Deck water lives on horizontal wood: foot traffic, standing water,
  // full sun. Quoting fence lifespans there overstates cost-per-year.
  ...
  __doc.getElementById('tierCards').innerHTML = ['essential', 'performance', 'showcase'].map(t => {
    ...
    // Each tier carries its own color-coded feature badge (Budget
    // Friendly = green / Most Color Options = blue / Longest-Lasting =
    // gold). No tier is auto-selected — a card only shows selected once
    // the user explicitly clicks it (tierConfirmed); the Next button
    // stays disabled until then.
    const badgeText = tm.badge || '';
    ...
    if (product === 'oil' && t === 'essential') {
      ...
      included = [
        '✓ <strong>SW Acrylic Alkyd Stain</strong> (SuperDeck Acrylic-Alkyd Solid Color)',
        '✓ <strong>Up to 15-year manufacturer warranty</strong> on fences (per Sherwin-Williams)',
        '✓ Siding &amp; hardware protection during application',
        '✓ Free 30-day touch-up visit',
        '✓ Fully insured &amp; licensed work'
      ];
    ...
    return `
      <button class="tier-card ${isSelected ? 'selected' : ''}" data-tier="${t}" data-mobile-collapse="true">
  ...
  __doc.querySelectorAll('#tierCards .tier-card').forEach(card => {
    card.addEventListener('click', (e) => {
      ...
      __doc.getElementById('stage6Next').innerHTML = shouldSkipColorStage() ? 'Next<span class="nav-next-suffix">: Add-ons</span> <span class="arr-r">→</span>' : 'Next<span class="nav-next-suffix">: Color</span> <span class="arr-r">→</span>';
      updateRunningTotal();
    });
  });
...
function renderHoaTierCards() {
  ...
  __doc.getElementById('stage6Next').disabled = false;
  __doc.getElementById('stage6Next').innerHTML = 'Next<span class="nav-next-suffix">: Add-ons</span> <span class="arr-r">→</span>';
}
...
function fenceTierRate(tier, productType) {
  const byProduct = PRICING.fence.tiersByProduct && PRICING.fence.tiersByProduct[productType];
...
function renderColorStage() {
  ...
  __doc.getElementById('colorTitle').textContent = `Pick a color — ${deckWaterColors ? 'SW SuperDeck (Water-Based)' : lib.line}`;
  ...
  const libNote = deckWaterColors
    ? 'Sherwin-Williams tints the SuperDeck deck base to this solid-color range. Hex values are approximations. Confirm against a physical sample.'
    : (lib.note || '');
...
function renderAddons() {
  ...
  // into SuperDeck Timber Oil. Drop it from state too so it can't linger
  // as a phantom charge after a tier switch.
...
function addonRow(a, group) {
  ...
  // Description is wrapped in .cust-mc so it stays hidden on mobile until
  // the user taps the ⓘ info button. Keeps each addon row compact (image +
  // name + price chip) so nothing overflows the viewport on a phone. The
  // info button stops propagation so the row's checkbox toggle doesn't fire.
  const infoBtn = a.desc
    ? `<button type="button" class="addon-info-btn" onclick="event.stopPropagation(); this.closest('.toggle-row').classList.toggle('expanded');" aria-label="More info">i</button>`
    : '';
  const descHtml = a.desc
    ? `<div class="addon-desc"><div class="ad-name-row"><div class="ad-name">${a.name}${restr}</div>${infoBtn}</div><div class="ad-sub cust-mc">${a.desc}</div></div>`
    : `<div class="addon-desc"><div class="ad-name-row"><div class="ad-name">${a.name}${restr}</div></div></div>`;
  return `<div class="toggle-row ${checked ? 'checked' : ''}" data-addon="${a.id}" data-group="${group}" data-mobile-collapse="true"><span class="box"></span>${imageHtml}${descHtml}${qtyInput}<span class="price">${priceLabel}</span></div>`;
}
...
function renderFinalBreakdown() {
        ...
        ${state.bundledProjects.map((p, i) => `
          <div class="saved-project-row" data-mobile-collapse="true" onclick="toggleMobileExpand(event)" role="button" tabindex="0">
            ...
            <div class="row-actions">
              <button type="button" class="edit-btn" data-edit-bundle="${i}" onclick="event.stopPropagation(); editBundledProject(${i});">Edit</button>
              <button type="button" class="remove-btn" data-remove-bundle="${i}" onclick="event.stopPropagation(); removeBundledProject(${i});">Remove</button>
            </div>
          </div>
        `).join('')}
      </div>`;
  ...
  const tierMeta = getTierMeta(state.activeProject.productType, tierName, proj);
  ...
  __doc.getElementById('breakdownMain').innerHTML = `
    ...
    <div class="quote-expiry-banner" style="margin-top:16px;">
      ...
      <div>This quote is locked through <strong>${getQuoteExpiryDate()}</strong> for 30 days. Your Sherwin-Williams Referral discount is already applied.</div>
    </div>
    ...
    <div class="risk-reversal-box">
      ...
      <ul>
        ...
        <li><strong>We don't get paid until you're happy</strong> — only 25% deposit at scheduling reserves your slot; balance is due after completion.</li>
        <li><strong>Quote is final &amp; transparent</strong> — no hidden fees, no surprise upsells on-site</li>
      </ul>
    </div>

    <div class="payment-options" style="margin-top: 12px; padding: 14px; background: var(--cream); border-radius: 10px; font-size: 13px; color: var(--slate);">
      <strong style="color: var(--navy); font-size: 14px;">Payment options:</strong>
      <div class="pay-opt-grid" style="margin-top:8px; display:flex; flex-direction:column; gap:8px;">
        <label class="pay-opt" style="display:flex; align-items:flex-start; gap:10px; padding:10px 12px; background:var(--paper); border:1.5px solid var(--line); border-radius:8px; cursor:pointer;">
          ...
          <div style="flex:1;">
            <div class="pay-opt-title" style="font-weight:700; color:var(--navy);">25% deposit + balance on completion</div>
            <div class="pay-opt-detail cust-mc" style="margin-top:6px; font-size:13px; line-height:1.5;">
              <strong style="color:var(--navy);">Deposit: $${Math.round(totals.finalTotal * 0.25).toLocaleString()}</strong> at scheduling. Remaining $${Math.round(totals.finalTotal * 0.75).toLocaleString()} due after we finish.
            </div>
          </div>
        </label>
        <label class="pay-opt" style="display:flex; align-items:flex-start; gap:10px; padding:10px 12px; background:var(--paper); border:1.5px solid var(--line); border-radius:8px; cursor:pointer;">
          ...
          <div style="flex:1;">
            <div class="pay-opt-title" style="font-weight:700; color:var(--navy);">25% deposit + Wisetack financing <span class="payment-pill" style="margin-left:6px;">0% APR available</span></div>
            <div class="pay-opt-detail cust-mc" style="margin-top:6px; font-size:13px; line-height:1.5;">
              <strong style="color:var(--navy);">Deposit: $${Math.round(totals.finalTotal * 0.25).toLocaleString()}</strong> at scheduling. ≈ $${Math.round((totals.finalTotal * 0.75)/24).toLocaleString()}/mo over 24 months, soft credit pull.
              <div style="font-size:11px; margin-top:4px;">*Final rate determined by Wisetack credit check.</div>
            </div>
          </div>
        </label>
      </div>
...
function computeDIYComparison(proTotal) {
  ...
  // the counter for that specific stain. SW edition: all tiers are SW
...
function editBundledProject(idx) {
  if (!confirm('Edit this project? Any project you have in progress will be saved first.')) return;
  if (state.activeProject.type) {
    const totals = computeProjectTotal();
    const cached = JSON.parse(JSON.stringify(state.activeProject));
    cached._cached = totals;
...
function removeBundledProject(idx) {
  if (!confirm('Remove this project from your estimate?')) return;
  state.bundledProjects.splice(idx, 1);
  renderFinalBreakdown();
  updateRunningTotal();
}
...
const INFO_TOPICS = {
  ...
  water_vs_oil: {
    ...
    body: `<p><strong>Water-based stains</strong> (like SW Woodscapes) dry fast (1–4 hours between coats), have low odor, clean up with water, and re-coat easily down the road. They lay on top of the wood more than they penetrate. Best when the wood was previously stained with water-based, or when you want low VOC / fast turnaround.</p>
      <p><strong>Oil-based stains</strong> (like Sherwin-Williams SuperDeck and Woodscapes Oil-Based) penetrate deeper into the wood, bring out grain more vividly, and resist UV breakdown longer — particularly important in the Southeast where summer sun is harsh. They typically last 30–50% longer per coat than water-based, but have stronger odor during application and a slower dry time (24 hours between coats vs. 4–6 hours for water-based).</p>
...
function buildJobberLineItem(p, idx, total) {
  ...
  // Water-based DECKS get the SuperDeck line — Woodscapes is a
  // vertical house stain and would peel on horizontal boards, so the
  ...
  const WARRANTY_BY_TIER = {
    ...
    'performance-water': '10-year limited manufacturer warranty (SW Woodscapes Rain Refresh, self-cleaning)',
    'showcase-water':    'Up to 15-year manufacturer warranty on fences (SW Acrylic-Alkyd, with proper prep)',
...
if (typeof window !== 'undefined') {
  ...
  customElements.define('sss-customer-calculator-sw', SSSCustomerCalculator);
})();
