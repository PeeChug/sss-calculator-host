...
const PRICING = {
  ...
  stainUpgrades: [
    ...
    { id: 'custom_color',name: 'Custom color match (you provide a sample or code)', priceType: 'flat', rate: 0,
      ...
      desc: 'Bring a paint chip, a photo, or a Sherwin-Williams color code. For water-based stains we use the SW color-match tool directly. For oil-based custom colors we work with the nearest EXPERT swatch.' }
  ],
  ...
  try {
    ...
    if (app) {
      const isRealInput = function (t) {
        if (!t) return false;
        if (t.id === 'custWebsiteHoneypot') return false; // hidden anti-bot field
      ...
      const syncFocusClass = function () {
        // Use the shadow root's activeElement (shadow DOM has its own
        // focus tracking). Falls back to document.activeElement if the
        // shadow root doesn't expose one (Wix Custom Element older
        // engines). Either way, if nothing real is focused, drop the
        // class so the nav reappears.
      ...
      // Recover from missed events: clear the class on any tap that
      // ISN'T on an input. Captures clicks on cards, buttons, nav
      // arrows — anywhere that should re-show the bottom bar.
      ...
      // Expose for showStage() to invoke after every transition.
...
function renderTierCards() {
  ...
  __doc.getElementById('tierCards').innerHTML = ['essential', 'performance', 'showcase'].map(t => {
    ...
    return `
      <button class="tier-card ${highlightCard ? 'recommended' : ''} ${isSelected ? 'selected' : ''}" data-tier="${t}" data-mobile-collapse="true">
...
function computeDIYComparison(proTotal) {
  ...
  // Accumulate stain cost by distinct product so the aggregate DIY row
  // can show one line per product (handles multi-project quotes that
  // mix tiers/products).
