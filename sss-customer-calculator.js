...
// =============================================================
// Hybrid approach so the customer can BOTH watch the auto-scroll AND
// swipe through manually:
//
//   - The row uses native overflow-x:auto (so user touches/swipes work
//     normally with momentum + scroll snapping).
//   - A requestAnimationFrame loop nudges scrollLeft by ~1px/frame so
//     when the user isn't touching, the row drifts at a slow creep.
//   - We duplicate the track's tiles once at startup so the row has 2×
//     content. When scrollLeft passes `half` (or goes below 0), we
//     wrap by adding/subtracting half — visually invisible because the
//     duplicates sit at the same x-position as the originals.
//   - Touch handling: on touchstart we set a "paused" flag, freezing
//     the JS driver while native scroll handles the user's swipe. On
//     touchend we wait 2 seconds, sync the accumulator to whatever
//     scrollLeft the user landed on, then resume drifting from there.
//
// Sub-pixel accumulator: scrollLeft gets rounded to integers in many
// browsers, so we keep an internal `accum` float and round at assign.
// Without this, at very slow speeds the int round-down eats the
// fractional motion and the row appears stuck.
function startHeroMarquees() {
  if (typeof window === 'undefined' || window.innerWidth >= 641) return;
  // Honour reduced-motion preferences — skip auto-scroll entirely.
  try {
    if (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
      console.log('[Customer] reduced-motion preference set — skipping gallery marquee');
      return;
    }
  } catch (e) {}
  ...
  rows.forEach(function (row) {
    ...
    // Slow creep — pixels per second. Decks slightly faster than fences
    // so the two rows don't sync visually.
    const pxPerSec = isReverse ? -6 : 8;

    // Duplicate originals once so the track has 2× content. Required
    // for the seamless wrap.
    ...
    // Sub-pixel accumulator (scrollLeft is rounded to int by browsers,
    // so slow speeds get truncated to 0 per frame without this).
    let accum = 0;
    let inited = false;
    let pauseUntil = 0;   // performance.now() timestamp; while < this, drift is frozen
    let isTouching = false;
    let resumeTimer = null;
    let lastTime = 0;

    function getHalf() {
      // Track scrollWidth = full duplicated content; half = original content width.
      // Sub 1 to avoid float-vs-int edge cases at the wrap point.
      return Math.max(1, track.scrollWidth / 2);
    }

    // Try to init repeatedly until layout settles (scrollWidth > 100).
    // Without this the first scrollWidth read might be 0 (images haven't
    // loaded yet, layout pass not done) and we'd compute a bogus half.
    function tryInit(attempts) {
      ...
      if (isReverse) {
        accum = half;
        row.scrollLeft = Math.round(half);
      } else {
        accum = 0;
        row.scrollLeft = 0;
      }
    ...
    function tick(now) {
      ...
      lastTime = now;
      if (!inited) { requestAnimationFrame(tick); return; }

      // Skip the drift while the user is touching OR during the resume-
      // delay window — they get full control of the native scroll.
      if (isTouching || now < pauseUntil) {
        requestAnimationFrame(tick);
        return;
      }
      ...
      const half = getHalf();
      accum += pxPerSec * dt;
      // Wrap: keep accum in [0, half). Done first against the float
      // accumulator so the wrap point is exact regardless of int rounding.
      if (accum >= half) accum -= half;
      else if (accum < 0) accum += half;
      // Assign rounded value to actual scroll position.
      row.scrollLeft = Math.round(accum);
      requestAnimationFrame(tick);
    }

    // Touch: pause drift while finger is down so native scroll can take
    // over. After release, wait 2s then sync accum to where the user
    // ended up and resume drifting from there.
    ...
    function scheduleResume() {
      ...
      resumeTimer = setTimeout(function () {
        // Sync float accum to wherever the user's scroll ended up so
        // the drift continues smoothly from that point.
        accum = row.scrollLeft;
        // Normalize back into [0, half) in case they scrolled past.
        const half = getHalf();
        if (accum >= half) accum = accum % half;
        else if (accum < 0) accum = (accum % half) + half;
        pauseUntil = 0;
      }, 2000);
    }
    row.addEventListener('touchend',    scheduleResume, { passive: true });
    row.addEventListener('touchcancel', scheduleResume, { passive: true });

    // Kick off init (with retry until layout is ready).
    tryInit(0);
  });
}
