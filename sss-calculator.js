...
async function pushFinishedQuoteToJobber(rowId, force, sendMethod) {
  ...
  if (box) {
    const verb = sendMethod === 'sms' ? 'Pushing &amp; texting the customer…'
               : sendMethod === 'email' ? 'Pushing &amp; emailing the customer…'
               : sendMethod === 'none' ? 'Uploading to Jobber (no customer send)…'
